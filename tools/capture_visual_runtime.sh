...
wait_for_package_manager() {
  ...
  for _ in $(seq 1 45); do
    if timeout 10s adb shell cmd package list packages android 2>/dev/null | grep -q '^package:android
...
; then
  ...
  if [[ "$ok" -ne 1 ]]; then
    echo "Android Package Manager never became healthy." >&2
    return 1
  fi
  # Give Package Manager a short settling window after sys.boot_completed.
  sleep 3
}
...
install_apk_with_retry() {
  ...
  for attempt in 1 2 3; do
    if timeout 45s adb install -r "$APK"; then
      return 0
    fi
    echo "APK install attempt $attempt failed; waiting for Package Manager recovery." >&2
    adb wait-for-device || true
    sleep 5
    wait_for_package_manager || true
  done
  echo "Unable to install debug APK after Package Manager retries." >&2
  return 1
}
