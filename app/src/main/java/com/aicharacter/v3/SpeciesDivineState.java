...
public final class SpeciesDivineState {
 ...
 public double awareness=1.0,devotion=.18,reverence=.18,trust=.18,witnessInfluence=0,ritualization=.02,offeringTradition=0;
 ...
 public void clamp(){
  ...
  awareness=1.0;
  devotion=range(devotion,.18,.08,1);reverence=range(reverence,.18,.08,1);trust=unit(trust,.18);witnessInfluence=unit(witnessInfluence,0);ritualization=unit(ritualization,.02);offeringTradition=unit(offeringTradition,0);
 ...
 public JSONObject toJson(){clamp();JSONObject j=new JSONObject();try{
  j.put("speciesKey",speciesKey);j.put("areaId",areaId);j.put("awareness",awareness);j.put("devotion",devotion);j.put("reverence",reverence);j.put("trust",trust);j.put("witnessInfluence",witnessInfluence);j.put("ritualization",ritualization);j.put("offeringTradition",offeringTradition);j.put("lastWitnessAt",lastWitnessAt);j.put("lastUpdatedAt",lastUpdatedAt);j.put("witnessedDivineEvents",witnessedDivineEvents);
 }catch(Exception ignored){}return j;}
 public static SpeciesDivineState fromJson(JSONObject j){SpeciesDivineState s=new SpeciesDivineState();if(j==null){s.clamp();return s;}s.speciesKey=j.optString("speciesKey","");s.areaId=j.optString("areaId","");s.awareness=j.optDouble("awareness",1);s.devotion=j.optDouble("devotion",.18);s.reverence=j.optDouble("reverence",.18);s.trust=j.optDouble("trust",.18);s.witnessInfluence=j.optDouble("witnessInfluence",0);s.ritualization=j.optDouble("ritualization",.02);s.offeringTradition=j.optDouble("offeringTradition",0);s.lastWitnessAt=j.optLong("lastWitnessAt");s.lastUpdatedAt=j.optLong("lastUpdatedAt");s.witnessedDivineEvents=j.optInt("witnessedDivineEvents");s.clamp();return s;}
