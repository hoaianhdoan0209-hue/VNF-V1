...
public final class GirlAnimationController{
 ...
 public static Visual select(WorldState s){
  ...
  if(a.contains("sitting")||a.contains("resting")||a.contains("reflect"))return v(State.SIT,"girl_sit_right",12,1.8f,.50f,.94f,"simulation sitting/resting");
  ...
  if(a.contains("think")||a.contains("looking")||a.contains("observe"))return v(State.THINK,"girl_think_right",12,2.0f,.50f,.94f,"attention/thought state");
  if(a.contains("react")||a.contains("reunion")||a.contains("wait")||a.contains("notice"))return v(State.REACT,"girl_react_right",12,2.8f,.50f,.94f,"grounded reaction state");
  ...
  return facingRight?v(State.IDLE,"girl_idle_right",12,2.0f,.50f,.94f,"no movement/action requiring another pose"):v(State.IDLE,"girl_idle_left",12,2.0f,.50f,.94f,"no movement/action requiring another pose");
 }
