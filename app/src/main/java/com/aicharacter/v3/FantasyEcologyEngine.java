...
public final class FantasyEcologyEngine{
 ...
 public static double areaUtility(WorldState s,WorldObject self,WorldArea area){
  ...
  double habitat=EcologyEngine.creatureSuitability(s,area,self),resource=resourceAvailability(s,self,area),preferred=tagAffinity(p.preferredAreaTags,area.tags)+tagAffinity(p.preferredAreaTags,s.world.biome(area.biomeId)==null?"":s.world.biome(area.biomeId).tags);
  double relation=relationSignal(s,self,area,p),population=PopulationEcologyEngine.overcrowding(s,self,area);return cl(habitat*.52+resource*p.resourceAffinity*.26+preferred*.18+relation*.22-population*.22);
 }
 public static double resourceAvailability(WorldState s,WorldObject self,WorldArea area){if(s==null||s.world==null||self==null||area==null)return 0;SpeciesEcologyProfile p=FantasyEcologyDictionary.forObject(self);if(p==null)return LivingWorldEngine.field(s,area.id).resourcePulse;BiomeProfile b=s.world.biome(area.biomeId);String context=(area.tags==null?"":area.tags)+","+(b==null?"":b.tags)+","+(b==null?"":b.vegetation);double signature=tagAffinity(p.resourceTags,context);for(WorldObject o:s.world.objects){if(!area.id.equals(o.areaId)||!LivingWorldEngine.isLivingFlora(o))continue;String identity=(o.tags==null?"":o.tags)+","+(o.dictionaryRef==null?"":o.dictionaryRef);signature=Math.max(signature,tagAffinity(p.resourceTags,identity));}double pulse=LivingWorldEngine.field(s,area.id).resourcePulse;return cl(pulse*(.28+.72*signature));}
 public static double consumeSpeciesResource(WorldState s,WorldObject self,String areaId,double requested){if(s==null||self==null||areaId==null)return 0;WorldArea area=s.world.area(areaId);if(area==null)return 0;double available=resourceAvailability(s,self,area),take=LivingWorldEngine.consumeFieldResource(s,areaId,requested*available);return take;}
 ...
 public static boolean stepReedlingMigration(WorldState s,WorldObject self,CreatureState c,double minutes){
  ...
  float dir=target.left>=cur.right?1f:target.right<=cur.left?-1f:(target.left+target.right>cur.left+cur.right?1f:-1f);double dt=minutes*60.0,desired=dir*(.10+.24*c.energy+.16*c.migrationIntent),traction=Math.max(.34,Math.min(.96,.90-EcologyEngine.localMoisture(s,cur)*.14-Math.abs(GroundGeometry.slope(s,c.x))*.38)),maxAccel=WholeBodyPhysicsEngine.GRAVITY*traction*.44,dv=desired-c.velocityXMps;c.velocityXMps+=Math.max(-maxAccel*dt,Math.min(maxAccel*dt,dv));c.heading=c.velocityXMps<-.006?-1:c.velocityXMps>.006?1:c.heading;float nx=(float)(c.x+WorldUnits.mToPx(c.velocityXMps*dt));
 ...
 public static boolean stepMigration(WorldState s,WorldObject self,CreatureLifeState c,double minutes){
  ...
  if(c.ecologyTargetAreaId.isEmpty()||c.migrationIntent<.30)return false;WorldArea target=s.world.area(c.ecologyTargetAreaId);if(target==null||!cur.connections.contains(target.id)){c.ecologyTargetAreaId="";return false;}
  float dir=target.left>=cur.right?1f:target.right<=cur.left?-1f:(target.left+target.right>cur.left+cur.right?1f:-1f);double dt=minutes*60.0,desired=dir*(.11+.30*c.locomotionDrive+.18*c.migrationIntent),traction=CreaturePhysicsEngine.isFlyer(self)?1.0:Math.max(.30,Math.min(.96,.88-EcologyEngine.localMoisture(s,cur)*.16-Math.abs(GroundGeometry.slope(s,c.x))*.42)),maxAccel=WholeBodyPhysicsEngine.GRAVITY*traction*(CreaturePhysicsEngine.isFlyer(self)?.30:.48),dv=desired-c.velocityXMps;c.velocityXMps+=Math.max(-maxAccel*dt,Math.min(maxAccel*dt,dv));c.heading=c.velocityXMps<-.006?-1:c.velocityXMps>.006?1:c.heading;float nx=(float)(c.x+WorldUnits.mToPx(c.velocityXMps*dt));
  if(dir>0&&nx>=cur.right-2){c.areaId=target.id;c.x=Math.min(target.right-24,target.left+24);c.ecologyTargetAreaId="";c.migrationIntent*=.45;return true;}
  if(dir<0&&nx<=cur.left+2){c.areaId=target.id;c.x=Math.max(target.left+24,target.right-24);c.ecologyTargetAreaId="";c.migrationIntent*=.45;return true;}
  c.x=Math.max(cur.left+22,Math.min(cur.right-22,nx));return true;
 }
