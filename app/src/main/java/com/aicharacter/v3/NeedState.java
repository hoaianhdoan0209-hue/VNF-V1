...
public final class NeedState {
 ...
 public static NeedState evaluate(WorldState s){
  ...
  double stress=s.endocrine==null?0:s.endocrine.stressResponse,skinDiscomfort=s.skin==null?0:Math.max(s.skin.irritation,s.skin.thermoregulationStrain),inflammation=s.immune==null?0:s.immune.inflammation;
  n.rest=(100-s.body.energy)*.65+s.body.sleepiness*.55+s.body.pain*.5+s.body.injuryBurden()*18+cold*12+heat*10+breath*9+n.hunger*.10+n.thirst*.18+stress*14+inflammation*12;
  n.physicalComfort=wet*55+rainExposure*42+s.body.pain*.25+Math.max(0,35-s.body.energy)*.12+cold*48+heat*42+breath*28+n.thirst*.22+n.elimination*.32+skinDiscomfort*24;
