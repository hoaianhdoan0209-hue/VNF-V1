...
public final class GodContactScene{
 ...
 public GodContactScene(Activity a,String inbox,Host h){
  ...
  LinearLayout top=new LinearLayout(a);top.setGravity(Gravity.CENTER_VERTICAL);TextView title=text("THẦN ĐANG HIỆN DIỆN",16,Color.rgb(242,225,178));title.setLetterSpacing(.08f);top.addView(title,new LinearLayout.LayoutParams(0,-2,1));Button x=new Button(a);x.setText("×");x.setOnClickListener(v->{if(!sending&&!closing)dismiss(host::onClose);else Toast.makeText(a,"Sự hiện diện chưa ổn định…",Toast.LENGTH_SHORT).show();});top.addView(x,new LinearLayout.LayoutParams(dp(56),dp(46)));c.addView(top);
  ...
  LinearLayout row=new LinearLayout(a);row.setGravity(Gravity.CENTER_VERTICAL);input=new EditText(a);input.setHint("Nói vào sự hiện diện…");input.setTextColor(Color.WHITE);input.setHintTextColor(Color.rgb(176,183,190));input.setMaxLines(3);row.addView(input,new LinearLayout.LayoutParams(0,-2,1));send=new Button(a);send.setText("GỬI");send.setOnClickListener(v->{String q=input.getText().toString().trim();if(!q.isEmpty()&&!sending&&!closing){busy(true);setPhase(DivineManifestationView.Phase.THINKING);conversation.append("\n\nMÈO · "+q+"\n\nTHẦN · …");bottom();host.onSend(q);}});row.addView(send,new LinearLayout.LayoutParams(dp(88),-2));c.addView(row);bottom();syncSession(GodSessionManager.getState());
 }
 ...
 public void showError(String s){setPhase(DivineManifestationView.Phase.WARNING);finish("Kết nối chao đảo. "+s,true);}
 ...
 private String label(GodSessionManager.State s){switch(s){case CONNECTING:return"✦ SỰ HIỆN DIỆN ĐANG KẾT NỐI";case ONLINE:return"✦ SỰ HIỆN DIỆN ỔN ĐỊNH";case DEGRADED:return"✦ SỰ HIỆN DIỆN CHẬP CHỜN";default:return"✦ CHỈ CÒN DƯ ÂM OFFLINE";}}
