...
public final class GodContactScene{
 ...
 public GodContactScene(Activity a,String inbox,Host h){
  this.a=a;host=h;root=new FrameLayout(a);root.setClickable(true);root.setBackgroundColor(Color.argb(24,5,10,20));root.setAlpha(0f);root.animate().alpha(1f).setDuration(520).start();
  ...
  LinearLayout top=new LinearLayout(a);top.setGravity(Gravity.CENTER_VERTICAL);TextView title=text("✦",18,Color.rgb(242,225,178));title.setLetterSpacing(.18f);top.addView(title,new LinearLayout.LayoutParams(0,-2,1));Button x=new Button(a);x.setText("×");x.setOnClickListener(v->{if(!sending&&!closing)dismiss(host::onClose);else Toast.makeText(a,"Không gian vẫn đang dao động…",Toast.LENGTH_SHORT).show();});top.addView(x,new LinearLayout.LayoutParams(dp(56),dp(46)));c.addView(top);
  ...
  scroll=new ScrollView(a);conversation=text(inbox==null?"":inbox,14,Color.rgb(234,232,218));conversation.setPadding(dp(15),dp(11),dp(15),dp(11));GradientDrawable panel=new GradientDrawable();panel.setColor(Color.argb(108,10,18,31));panel.setCornerRadius(dp(10));conversation.setBackground(panel);scroll.addView(conversation);c.addView(scroll,new LinearLayout.LayoutParams(-1,dp(96)));
  LinearLayout row=new LinearLayout(a);row.setGravity(Gravity.CENTER_VERTICAL);input=new EditText(a);input.setHint("Nói trong thế giới…");input.setTextColor(Color.WHITE);input.setHintTextColor(Color.rgb(176,183,190));input.setMaxLines(3);row.addView(input,new LinearLayout.LayoutParams(0,-2,1));send=new Button(a);send.setText("GỬI");send.setOnClickListener(v->{String q=input.getText().toString().trim();if(!q.isEmpty()&&!sending&&!closing){busy(true);setPhase(DivineManifestationView.Phase.THINKING);conversation.append("\n\nMÈO · "+q+"\n\nTHẦN · …");bottom();host.onSend(q);}});row.addView(send,new LinearLayout.LayoutParams(dp(88),-2));c.addView(row);bottom();syncSession(GodSessionManager.getState());
 }
