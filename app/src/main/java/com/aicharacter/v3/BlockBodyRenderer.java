...
public final class BlockBodyRenderer{private BlockBodyRenderer(){}
 public static void drawHuman(Canvas c,Paint p,WorldState s,float x,float ground){
  ...
  float shoulderX=pelvisX+spine*34,shoulderY=pelvisY-74*sc,headX=shoulderX+neck*22,headY=shoulderY-45*sc;
  float phase=(float)(r.stridePhase*Math.PI*2),swing=(float)Math.sin(phase)*24;
  float lHip=pelvisX-12,rHip=pelvisX+12,lFoot=r.leftFootContact>.5&&Double.isFinite(r.leftFootWorldX)?(float)(r.leftFootWorldX-s.haruX+x):x-15+swing,rFoot=r.rightFootContact>.5&&Double.isFinite(r.rightFootWorldX)?(float)(r.rightFootWorldX-s.haruX+x):x+15-swing;
  float kneeY=ground-42*sc,lKnee=(lHip+lFoot)*.5f+(float)j.knee*9,rKnee=(rHip+rFoot)*.5f-(float)j.knee*9;
  p.setStrokeWidth(13);p.setStrokeCap(Paint.Cap.ROUND);p.setColor(Color.rgb(67,72,78));line(c,p,lHip,pelvisY,lKnee,kneeY);line(c,p,lKnee,kneeY,lFoot,ground);line(c,p,rHip,pelvisY,rKnee,kneeY);line(c,p,rKnee,kneeY,rFoot,ground);
  p.setStrokeWidth(25);p.setColor(Color.rgb(92,111,116));line(c,p,pelvisX,pelvisY,shoulderX,shoulderY);
  p.setStrokeWidth(10);p.setColor(Color.rgb(78,84,91));float arm=(float)j.shoulder*22;line(c,p,shoulderX-18,shoulderY,shoulderX-25-arm,shoulderY+65);line(c,p,shoulderX+18,shoulderY,shoulderX+25+arm,shoulderY+65);
