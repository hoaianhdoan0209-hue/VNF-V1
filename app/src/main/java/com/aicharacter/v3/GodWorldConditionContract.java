...
public final class GodWorldConditionContract {
 ...
 public static boolean markApplied(WorldState s,String id,String consumerToken,long now){WorldConditionProposalState p=get(s,id);if(p==null||!"PROPOSED".equals(p.status))return false;p.status="APPLIED";p.consumerToken=consumerToken==null?"":consumerToken;p.appliedAt=now;WorldEventBus.publishId(s,now,"god_world_applied_"+safeId(id)+"_"+Long.toHexString(now),"GOD_WORLD_CONDITION_APPLIED",id,"condition="+p.condition+" scope="+p.scopeType+":"+p.scopeTarget+" intensity="+fmt(p.intensity)+" status=APPLIED");return true;}
 ...
 public static boolean markRolledBack(WorldState s,String id,String consumerToken,long now){WorldConditionProposalState p=get(s,id);if(p==null||!"ROLLBACK_REQUESTED".equals(p.status))return false;if(p.consumerToken!=null&&!p.consumerToken.isEmpty()&&consumerToken!=null&&!consumerToken.isEmpty()&&!p.consumerToken.equals(consumerToken))return false;p.status="ROLLED_BACK";p.rolledBackAt=now;WorldEventBus.publishId(s,now,"god_world_rolledback_"+safeId(id)+"_"+Long.toHexString(now),"GOD_WORLD_CONDITION_ROLLED_BACK",id,"condition="+p.condition+" scope="+p.scopeType+":"+p.scopeTarget+" status=ROLLED_BACK");return true;}
 ...
 private static String safeId(String s){return s==null?"unknown":s.replaceAll("[^A-Za-z0-9._-]","_");}\n private static String fmt(double d){return String.format(java.util.Locale.US,"%.2f",Double.isFinite(d)?d:0);}\n private static void ensure(WorldState s){if(s.characterGod==null)s.characterGod=new CharacterGodState();}
