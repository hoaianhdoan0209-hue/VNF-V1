...
public final class GodOnlineClient {
    ...
    public static void ask(Context appContext,WorldState state,String input,Callback callback){
        new Thread(() -> {
            ...
            for(int attempt=1;attempt<=2;attempt++){
                try{
                    callback.onResult(requestOnce(appContext,state,input),null);
                    return;
                }catch(Throwable t){
                    last=t;
                    if(attempt>=2||!retryable(t))break;
                    try{Thread.sleep(1500L);}catch(InterruptedException interrupted){Thread.currentThread().interrupt();last=interrupted;break;}
                }
            }
            callback.onResult(null,last==null?new IOException("Không nhận được phản hồi từ Thần"):last);
    ...
    private static Result requestOnce(Context appContext,WorldState state,String input) throws Exception{
        ...
        try{
            ...
            // Backend itself may legitimately spend up to ~30 s waiting for Gemini. Keep the Android client above that ceiling.
            c.setReadTimeout(38000);
            c.setDoOutput(true);
            c.setRequestProperty("Content-Type","application/json; charset=utf-8");
            c.setRequestProperty("Accept","application/json");
            c.setRequestProperty("X-VNF-Protocol","3");
            ...
            if(code<200||code>=300)throw new HttpFailure(code,"HTTP "+code+": "+safeError(raw));
            ...
            JSONObject patch=json.optJSONObject("contentPatch");
            String manifestUrl=patch==null?"":patch.optString("manifestUrl","").trim();
            boolean autoApply=patch!=null&&patch.optBoolean("autoApply",false);
            GodRepairPlan repairPlan=GodRepairPlan.fromJson(json.optJSONObject("repairPlan"));
            JSONObject recipe=json.optJSONObject("signedWorldRecipe");
            String gp=recipe==null?"":recipe.optString("payloadBase64","");
            String gs=recipe==null?"":recipe.optString("signatureBase64","");
            return new Result(reply,manifestUrl,autoApply,repairPlan,gp,gs);
        }finally{if(c!=null)c.disconnect();}
    }

    private static boolean retryable(Throwable t){
        if(t instanceof HttpFailure){int code=((HttpFailure)t).code;return code==408||code==425||code==429||code>=500;}
        return t instanceof SocketTimeoutException||t instanceof IOException;
    }
