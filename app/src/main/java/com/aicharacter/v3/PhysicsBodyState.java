...
public final class PhysicsBodyState{
 ...
 public JSONObject toJson(){normalizeFinite(massKg);JSONObject j=new JSONObject();try{j.put("massKg",massKg);j.put("velocityX",velocityX);j.put("velocityY",velocityY);j.put("centerOfMassX",centerOfMassX);j.put("centerOfMassY",centerOfMassY);j.put("groundClearanceM",groundClearanceM);j.put("groundReaction",groundReaction);j.put("traction",traction);j.put("balance",balance);j.put("supportLeft",supportLeft);j.put("supportRight",supportRight);j.put("landingInstability",landingInstability);j.put("slipVelocity",slipVelocity);j.put("slipSeverity",slipSeverity);j.put("fallBodyHeightM",fallBodyHeightM);j.put("grounded",grounded);j.put("falling",falling);j.put("lastUpdatedAt",lastUpdatedAt);}catch(Exception ignored){}return j;}
