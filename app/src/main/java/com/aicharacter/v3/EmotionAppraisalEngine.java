...
public final class EmotionAppraisalEngine {
 ...
 private static double catReturnBelief(WorldState s,MemoryEntry m){
  if(m==null||!explicitCatRelevant(m))return 0;BeliefState b=s.beliefStates==null?null:s.beliefStates.get("cat_returns");if(b==null)return 0;return "usually_returns".equals(b.value)||"returns".equals(b.value)||"likely_returns".equals(b.value)?cl01(b.confidence):.5*cl01(b.confidence);
 }
 private static double relationshipRelevance(WorldState s,MemoryEntry m){if(s.relationship==null)return 0;boolean social=explicitCatRelevant(m)||tag(m,"social")>0||tag(m,"kind")>0||tag(m,"respect")>0||tag(m,"control")>0||tag(m,"absence")>0;if(!social)return 0;double attachment=cl01(s.relationship.attachment/100.0),affection=cl01(s.relationship.affection/100.0),trust=cl01(s.relationship.trust/100.0);return cl01(.30+.32*attachment+.20*affection+.18*trust);}
 ...
 private static String targetOf(MemoryEntry m,Experience e){if(e!=null&&e.objectId!=null&&!e.objectId.isEmpty())return e.objectId;if(explicitCatRelevant(m))return"cat";return m.location==null?"":m.location;}
 private static boolean explicitCatRelevant(MemoryEntry m){if(m==null)return false;String k=m.kind==null?"":m.kind;return tag(m,"cat")>0||tag(m,"absence")>0||tag(m,"find_cat")>0||tag(m,"kind")>0||tag(m,"respect")>0||tag(m,"control")>0||k.contains("cat");}
