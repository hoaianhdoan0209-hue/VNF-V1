...
public final class HabitState {
 ...
 public double effectiveStrength(long now,String currentContext){double v=strength;if(lastPerformed>0&&now>lastPerformed){double days=(now-lastPerformed)/86400000.0;v*=Math.max(.35,Math.exp(-days/180.0));}if(context!=null&&!context.isEmpty()&&currentContext!=null&&!currentContext.isEmpty()&&!context.equals(currentContext))v*=.72;return Math.max(0,Math.min(1,v));}
