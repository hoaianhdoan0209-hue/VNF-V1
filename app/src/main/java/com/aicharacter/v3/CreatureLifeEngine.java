...
public final class CreatureLifeEngine {private CreatureLifeEngine(){}
 public static void advance(WorldState s,double minutes,long now){CreatureState c=s.reedling;if(c==null||s.world==null)return;WorldObject o=s.world.object(c.objectId);if(o==null)return;WorldArea a=s.world.area(c.areaId);if(a==null)a=s.world.area(o.areaId);if(a==null)return;
  ...
  c.hunger=Math.min(1,c.hunger+minutes*(.0012+.0007*(1-suitability)));BiomeProfile b=EcologyEngine.biome(s,a);double exposure=WorldSemantics.exposure(a),rain="RAIN".equals(s.environment.weather)?s.environment.weatherIntensity:0,canopy=b==null?0:b.canopy,risk=rain*exposure*(1-canopy*.55);
  if(suitability<.32){c.activity="rest";c.energy=Math.max(0,c.energy-minutes*.0012);}
  else if(c.hunger>.65&&suitability>.45){c.activity="forage";c.energy=Math.max(0,c.energy-minutes*.0015);c.hunger=Math.max(0,c.hunger-minutes*.00055*suitability);}
  else if(risk>.62||c.energy<.25){c.activity="rest";c.energy=Math.min(1,c.energy+minutes*.003);}
  ...
  c.areaId=a.id;c.lastUpdate=now;
 }
