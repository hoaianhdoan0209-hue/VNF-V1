...
public final class EnvironmentState {
 public String weather="CLEAR";public double weatherIntensity=.15,cloudCover=.15,wind=.12,ambientBrightness=.9;public long weatherSince,lastNaturalWeatherBucket=-1,lastAtmosphereUpdateAt=0;
 ...
 public JSONObject toJson(){JSONObject j=new JSONObject();try{j.put("weather",weather);j.put("weatherIntensity",weatherIntensity);j.put("cloudCover",cloudCover);j.put("wind",wind);j.put("ambientBrightness",ambientBrightness);j.put("weatherSince",weatherSince);j.put("lastNaturalWeatherBucket",lastNaturalWeatherBucket);j.put("lastAtmosphereUpdateAt",lastAtmosphereUpdateAt);}catch(Exception ignored){}return j;}
 public static EnvironmentState fromJson(JSONObject j){EnvironmentState e=new EnvironmentState();if(j==null)return e;e.weather=j.optString("weather","CLEAR").toUpperCase();e.weatherIntensity=j.optDouble("weatherIntensity",.15);e.cloudCover=j.optDouble("cloudCover",.15);e.wind=j.optDouble("wind",.12);e.ambientBrightness=j.optDouble("ambientBrightness",.9);e.weatherSince=j.optLong("weatherSince",0);e.lastNaturalWeatherBucket=j.optLong("lastNaturalWeatherBucket",-1);e.lastAtmosphereUpdateAt=j.optLong("lastAtmosphereUpdateAt",e.weatherSince);return e;}
}
