...
public final class WorldInteractionEngine {
 ...
 public static boolean interact(WorldState s,WorldObject o,String action,long now){if(o==null||!o.enabled||!o.interactable||!PhysicalInteraction.inRangeForAction(s,o,"REST".equals(action)?"REST_PROTECT":action))return false;String id=o.id;
  if("EAT".equals(action)&&o.tags.contains("food_source")){long now=Math.max(System.currentTimeMillis(),Math.max(s.lastOpenedAt,s.lastSimulatedAt));if(!DigestionHydrationEngine.eat(s,now))return false;s.haruActivity="eating a simple meal";remember(s,o,"ate a simple meal from the food available at home","eat",now);return true;}
  if("DRINK".equals(action)&&o.tags.contains("water_source")){long now=Math.max(System.currentTimeMillis(),Math.max(s.lastOpenedAt,s.lastSimulatedAt));if(!DigestionHydrationEngine.drink(s,now))return false;s.haruActivity="drinking water";remember(s,o,"drank water at home","drink",now);return true;}
  if("TOILET".equals(action)&&o.tags.contains("toilet")){long now=Math.max(System.currentTimeMillis(),Math.max(s.lastOpenedAt,s.lastSimulatedAt));if(!DigestionHydrationEngine.urinate(s,now))return false;s.haruActivity="taking care of a bodily need";remember(s,o,"used the private sanitation space at home","toilet",now);return true;}
