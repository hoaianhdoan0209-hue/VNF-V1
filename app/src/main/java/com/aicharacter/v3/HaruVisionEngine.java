...
public final class HaruVisionEngine{
 ...
 public static boolean canSee(WorldState s,WorldObject o){if(s==null||s.world==null||o==null||!o.enabled)return false;WorldArea area=s.world.areaAt(s.haruX);if(area==null||!area.id.equals(actualAreaId(s,o)))return false;double visibility=WorldSemantics.visibility(s,area),attention=attention(s);float range=(float)(120+520*visibility*attention);return Math.abs(actualX(s,o)-s.haruX)<=range;} public static float actualX(WorldState s,WorldObject o){return s!=null&&s.reedling!=null&&o!=null&&o.id.equals(s.reedling.objectId)?s.reedling.x:o==null?0:o.x;} public static String actualAreaId(WorldState s,WorldObject o){return s!=null&&s.reedling!=null&&o!=null&&o.id.equals(s.reedling.objectId)?s.reedling.areaId:o==null?"":o.areaId;}
 ...
 private static Seen seeObject(WorldState s,WorldArea area,WorldObject o,float range,double visibility,double attention){if(o==null||!o.enabled||!area.id.equals(actualAreaId(s,o)))return null;float ox=actualX(s,o),d=Math.abs(ox-s.haruX);if(d>range)return null;double clarity=Math.max(.12,Math.min(1,visibility*attention*(1-d/Math.max(1f,range)*.55)));String label=o.haruDescription==null||o.haruDescription.trim().isEmpty()?"một vật ở gần":o.haruDescription;if(LivingWorldEngine.isLivingFlora(o)){FloraLifeState f=LivingWorldEngine.flora(s,o.id);if(f!=null){if(f.opening<.30)label+=", đang khép lại nhẹ";else if(f.sense.vibrationSense>.48)label+=", đang rung phản ứng với nền đất";else if(f.swayDrive>.58)label+=", đang lay mạnh theo dòng khí";}}else if("creature".equals(o.type)&&s.reedling!=null&&s.reedling.objectId.equals(o.id)){if(s.reedling.sense!=null&&s.reedling.sense.alertness>.68)label+=", chuyển động cảnh giác";else if("rest".equals(s.reedling.activity))label+=", đang thu mình nghỉ";}return new Seen(o.id,label,o.type,direction(ox-s.haruX),d,clarity,familiar(s,o));}
