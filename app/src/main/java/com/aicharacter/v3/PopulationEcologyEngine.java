...
public final class PopulationEcologyEngine {
 ...
 private static WorldArea bestMigrationTarget(WorldState s,SpeciesEcologyProfile p,WorldArea from,SpeciesPopulationState source){WorldArea best=null;double bestGain=.08;source.clamp();for(String id:from.connections){WorldArea a=s.world.area(id);if(a==null)continue;String pk=key(p.key,a.id);SpeciesPopulationState t=s.livingWorld.populations.get(pk);if(t==null){double fit=profileSuitability(s,p,a,null);if(fit<VIABLE_SUITABILITY)continue;t=s.livingWorld.population(p.key,a.id);t.carryingCapacity=unit(.06+fit*.58,0);t.resourcePressure=unit(1-profileResourceAvailability(s,p,a,null),1);t.weatherPressure=weatherPressure(s,p,a);t.lastUpdatedAt=source.lastUpdatedAt;t.clamp();}else t.clamp();if(t.carryingCapacity<=0)continue;double gain=finite((t.carryingCapacity-t.relativeAbundance)-(source.carryingCapacity-source.relativeAbundance)+(.5-t.resourcePressure)*.16-(t.weatherPressure-source.weatherPressure)*.18,0);if(gain>bestGain){bestGain=gain;best=a;}}return best;}
 ...
 private static double discoverySuitability(WorldState s,SpeciesEcologyProfile p,WorldArea a){
  if(s==null||p==null||a==null)return 0;BiomeProfile b=s.world==null?null:s.world.biome(a.biomeId);String context=(a.tags==null?"":a.tags)+","+(b==null?"":b.tags)+","+(b==null?"":b.vegetation)+","+(b==null?"":b.fauna)+","+(b==null?"":b.waterRegime);
  double preferred=resourceOverlap(p.preferredAreaTags,context),resources=resourceOverlap(p.resourceTags,context);double stable=(((p.key+"@"+a.id).hashCode()&0x7fffffff)%1000)/1000.0;
  return unit(preferred*.62+resources*.24+.05+stable*.04,0);
 }
 private static double profileSuitability(WorldState s,SpeciesEcologyProfile p,WorldArea a,WorldObject rep){
  ...
  double stable=(((p.key+"@"+a.id).hashCode()&0x7fffffff)%1000)/1000.0;return unit(preferred*.62+resources*.24+pulse*.10+stable*.04,0);
 }
