...
final class WorldConditionProposalState {
 public String id="",condition="",scopeType="GLOBAL",scopeTarget="",provenanceLayer="GOD_PROPOSAL",sourceRef="",rollbackPolicy="RESTORE_PREVIOUS_BASELINE",status="PROPOSED",consumerToken="";
 ...
 public JSONObject toJson(){JSONObject j=new JSONObject();try{j.put("id",id);j.put("condition",condition);j.put("scopeType",scopeType);j.put("scopeTarget",scopeTarget);j.put("provenanceLayer",provenanceLayer);j.put("sourceRef",sourceRef);j.put("rollbackPolicy",rollbackPolicy);j.put("status",status);j.put("consumerToken",consumerToken);j.put("intensity",intensity);j.put("durationMinutes",durationMinutes);j.put("sourceConfidence",sourceConfidence);j.put("proposedAt",proposedAt);j.put("appliedAt",appliedAt);j.put("rollbackRequestedAt",rollbackRequestedAt);j.put("rolledBackAt",rolledBackAt);}catch(Exception ignored){}return j;}
 public static WorldConditionProposalState fromJson(String key,JSONObject j){WorldConditionProposalState p=new WorldConditionProposalState();p.id=j.optString("id",key);p.condition=j.optString("condition","");p.scopeType=j.optString("scopeType","GLOBAL");p.scopeTarget=j.optString("scopeTarget","");p.provenanceLayer=j.optString("provenanceLayer","GOD_PROPOSAL");p.sourceRef=j.optString("sourceRef","");p.rollbackPolicy=j.optString("rollbackPolicy","RESTORE_PREVIOUS_BASELINE");p.status=j.optString("status","PROPOSED");p.consumerToken=j.optString("consumerToken","");p.intensity=LessonState.unit(j.optDouble("intensity",0));p.durationMinutes=Math.max(1,Math.min(360,j.optDouble("durationMinutes",30)));p.sourceConfidence=LessonState.finite(j.optDouble("sourceConfidence",-1),-1);p.proposedAt=j.optLong("proposedAt");p.appliedAt=j.optLong("appliedAt");p.rollbackRequestedAt=j.optLong("rollbackRequestedAt");p.rolledBackAt=j.optLong("rolledBackAt");return p;}
}
