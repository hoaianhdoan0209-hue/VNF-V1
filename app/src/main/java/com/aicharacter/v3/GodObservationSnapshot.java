...
public final class GodObservationSnapshot {
 ...
 public static JSONObject build(WorldState s){
  ...
  try{
   ...
   JSONObject divine=new JSONObject();if(s.divineOntology!=null){s.divineOntology.clamp();divine.put("ontology","KNOWN_TO_ALL_LIFE");divine.put("worshipSustains","COGNITION_AND_ACTIVE_PRESENCE");divine.put("currentWorship",band(s.divineOntology.currentWorship));divine.put("historicalWorship",band(s.divineOntology.accumulatedWorship));divine.put("evidenceBoundKnowledge",true);divine.put("learnedKnowledgePersists",true);divine.put("cognitiveCapacity",s.divineOntology.capacity.qualitativeJson());divine.put("trackedSpeciesDevotion",s.livingWorld==null?0:s.livingWorld.divineStates.size());int chosen=0,activeGifts=0;for(DivineFollowerState f:s.divineOntology.followers.values())if(f!=null&&f.chosen)chosen++;for(DivineGiftState g:s.divineOntology.gifts.values())if(g!=null&&("GRANTED".equals(g.status)||"ACTIVE".equals(g.status)))activeGifts++;divine.put("chosenFollowers",chosen);divine.put("recordedOfferings",s.divineOntology.offerings.size());divine.put("activeOrGrantedGifts",activeGifts);divine.put("divinePower",band(s.divineOntology.power==null?0:s.divineOntology.power.available()/100.0));}root.put("divineOntology",divine);
